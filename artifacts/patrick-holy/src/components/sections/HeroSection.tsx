import { Link } from "wouter";
import { motion } from "framer-motion";
import { Button } from "@/components/ui/button";
import { Briefcase, Star } from "lucide-react";

const awards = [
  {
    id: "dvgw",
    label: "DVGW GW 301",
    content: (
      <img
        src="/images/dvgw-logo.png"
        alt="DVGW Zertifizierung GW 301"
        className="h-7 md:h-8 w-auto object-contain"
        style={{ filter: "brightness(0) invert(1)" }}
      />
    ),
  },
  {
    id: "tuv",
    label: "TÜV Geprüft",
    content: (
      <img
        src="/images/tuv-logo.png"
        alt="TÜV Rheinland Prüfung"
        className="h-7 md:h-8 w-auto object-contain"
        style={{ filter: "brightness(0) invert(1)" }}
      />
    ),
  },
  {
    id: "team",
    label: "40 Kollegen",
    content: (
      <div
        className="h-8 w-8 md:h-9 md:w-9 rounded-full flex items-center justify-center"
        style={{
          background:
            "radial-gradient(circle at 30% 25%, #ffd84a 0%, #febf05 55%, #c89500 100%)",
          boxShadow:
            "inset 0 1px 2px rgba(255,255,255,0.6), inset 0 -2px 4px rgba(0,0,0,0.25)",
        }}
      >
        <span className="font-extrabold text-[11px] md:text-xs text-[#213d86] tracking-tight">
          40
        </span>
      </div>
    ),
  },
  {
    id: "google",
    label: "4,8 Google",
    href: "https://www.google.com/search?q=Patrick+Holy+GmbH+Aschaffenburg",
    content: (
      <div className="flex items-center gap-1.5">
        <svg viewBox="0 0 24 24" className="w-5 h-5 md:w-6 md:h-6" aria-hidden="true">
          <path
            fill="#4285F4"
            d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
          />
          <path
            fill="#34A853"
            d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
          />
          <path
            fill="#FBBC05"
            d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
          />
          <path
            fill="#EA4335"
            d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
          />
        </svg>
        <span className="font-extrabold text-white text-sm md:text-base leading-none">4,8</span>
        <Star className="w-3.5 h-3.5 fill-yellow-400 text-yellow-400" />
      </div>
    ),
  },
] as const;

export default function HeroSection() {
  return (
    <section data-hero className="relative min-h-[90vh] flex items-center justify-center overflow-hidden">
      {/* Background Image */}
      <div className="absolute inset-0 z-0">
        <img
          src="/images/hero-baustelle.png"
          alt="Bauarbeiten Patrick Holy Aschaffenburg"
          className="w-full h-full object-cover"
        />
        <div className="absolute inset-0 bg-[#213d86]/80 mix-blend-multiply" />
        <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent" />
      </div>

      <div className="container mx-auto px-4 md:px-6 relative z-10 text-center text-white mt-20">
        <motion.div
          initial={{ opacity: 0, y: 30 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.8 }}
          className="max-w-4xl mx-auto"
        >
          <h1 className="text-4xl md:text-6xl lg:text-7xl font-extrabold tracking-tight mb-6 leading-tight">
            Ein sicherer Job direkt vor deiner Haustür
          </h1>

          <p className="text-xl md:text-2xl text-blue-100 mb-10 max-w-2xl mx-auto leading-relaxed">
            Unbefristete Anstellung, gesicherte Auftragslage und die Baustelle liegt in der Region,
            nicht in Frankfurt.
          </p>

          <div className="flex items-center justify-center">
            <Button
              asChild
              size="lg"
              className="w-72 sm:w-auto bg-accent text-accent-foreground hover:bg-accent/90 font-bold text-lg h-14 px-8"
            >
              <Link href="/karriere#offene-stellen" className="flex items-center gap-2">
                <Briefcase className="w-5 h-5" />
                Offene Stellen ansehen
              </Link>
            </Button>
          </div>

          <motion.div
            initial={{ opacity: 0, y: 12 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6, delay: 0.45 }}
            className="mt-12 flex flex-wrap items-center justify-center gap-x-6 gap-y-4 md:gap-x-10"
          >
            {awards.map((award) => {
              const className =
                "flex flex-col items-center gap-1.5 opacity-90 hover:opacity-100 transition-opacity";
              const inner = (
                <>
                  <div className="h-9 flex items-center justify-center" data-testid={`badge-${award.id}`}>
                    {award.content}
                  </div>
                  <span className="text-[10px] md:text-[11px] font-semibold tracking-wide uppercase text-blue-100/80">
                    {award.label}
                  </span>
                </>
              );

              if ("href" in award && award.href) {
                return (
                  <a
                    key={award.id}
                    href={award.href}
                    target="_blank"
                    rel="noopener noreferrer"
                    className={`${className} group`}
                  >
                    {inner}
                  </a>
                );
              }

              return (
                <div key={award.id} className={className}>
                  {inner}
                </div>
              );
            })}
          </motion.div>
        </motion.div>
      </div>
    </section>
  );
}
